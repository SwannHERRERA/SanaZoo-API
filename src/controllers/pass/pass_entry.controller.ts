...
export class EntryController {
  ...
  public async getEntriesByUserId(req: Request, res: Response): Promise<void> {
    ...
    for (const pass of passList) {
      const foundEntries: IEntry_Instance[] = await this.Entry.findAll({
        order: [["created_at", "DESC"]],
      });
  ...
  public async addEntry(req: Request, res: Response): Promise<void> {
    ...
    const passEnclosureAccessList: IPass_Enclosure_Access_Instance[] = await this.PassEnclosureAccess.findAll(
      { where: { passId } }
    );
    ...
    if (
      !passEnclosureAccessList.find((e) => {
        return e.enclosureId == enclosureId;
      })
    ) {
      res
        .status(403)
        .json("Your pass don't allow access to this enclosure")
        .end();
      return;
    }
  ...
  private async dayEntry(
    ...
    if (
      currentDate.getDate() != pass.validDate.getDate() ||
  ...
  private async weekendEntry(
    ...
    if (
      (currentDate.getTime() - pass.validDate.getTime()) /
        (1000 * 60 * 60 * 24) >
      7
    ) {
  ...
  private async oneDayMonthEntry(
    res: Response,
    currentDate: Date,
    pass: IPass_Instance,
    enclosure: IEnclosure_Instance
    ...
    if (
      ...
      res
        .status(403)
        .json("Your already have an entry for this month at another day")
        .end();
      return;
    }
    await this.createEntry(pass.id, enclosure.id, res);
  ...
  private async escapeGameEntry(
    res: Response,
    currentDate: Date,
    pass: IPass_Instance,
    enclosure: IEnclosure_Instance,
    passEnclosureAccessList: IPass_Enclosure_Access_Instance[]
  ) {
    ...
    if (!enclosureAccess.order) {
      res
        .status(403)
        .json("This is escape pass but enclosure access don't have order")
        .end();
      return;
    }
    ...
    if (enclosureAccess.order > 1) {
      const previousEnclosure:
        ...
        return (
          e.order &&
          enclosureAccess.order &&
          e.order == enclosureAccess.order - 1
        );
      });
      if (!previousEnclosure) {
        res
          .status(404)
          .json("Can't find previous enclosure access in this escape pass");
        return;
      }
      ...
      if (!previousEntry) {
        res
          .status(403)
          .json(
            "You must enter enclosure by order ! Missing entry for previous enclosure"
          )
          .end();
        return;
      }
  ...
  private async createEntry(
    ...
    const entry: IEntry_Instance = await this.Entry.create({
      passId,
      enclosureId,
    });
    res.status(200).json(entry).end();
