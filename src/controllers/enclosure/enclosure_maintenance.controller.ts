...
export class Enclosure_Maintenance_Controller {
  ...
  public static async getInstance(): Promise<Enclosure_Maintenance_Controller> {
    if (Enclosure_Maintenance_Controller.instance === undefined) {
      ...
      Enclosure_Maintenance_Controller.instance = new Enclosure_Maintenance_Controller(
        Entry
      );
    }
  ...
  public async getBestMonth(id: number): Promise<any> {
    ...
    const result = await manager.sequelize.query(
      "SELECT DATE_FORMAT(Entry.created_at,'%M') as month, count(*) as attendance FROM Entry WHERE CURDATE() - INTERVAL 1 YEAR < Entry.created_at GROUP BY month ORDER BY attendance ASC LIMIT 1",
