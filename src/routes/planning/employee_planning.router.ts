...
employeePlanningRouter.get("/:id", employeeMiddleware, async function(req, res) {
   const id = Number.parseInt(req.params.id);
   const controller = await Planning_Controller.getInstance();
...
employeePlanningRouter.put("/:id", employeeMiddleware, async function(req, res) {
    const id = Number.parseInt(req.params.id);
    const controller = await Planning_Controller.getInstance();
...
const schemaValidate = yup.object().shape({
    ...
    end_time: yup.date().when(
        'start_time',
        (start_time: Date) => (yup.date().min(start_time))).required(),
