...
passNightRouter.get('/', authMiddleware, async function (req, res) {
    await (await getPassNightController()).getAllPassNightsAvailabilities(req, res);
});
...
passNightRouter.get('/available', async function (req, res) {
    await (await getPassNightController()).getAvailablePassNightsAvailabilities(req, res);
});
